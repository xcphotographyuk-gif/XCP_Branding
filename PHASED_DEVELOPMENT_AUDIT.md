...
|------|----------------|---------------|-------------|-------------|------|
| Homepage | ✅ 5-part template | ⚠️ Placeholder images | ⚠️ CTA links to form only | ❌ Not in JSON | ❌ Missing consent |
...
| Contact | ✅ Complete template | ✅ Content defined | ❌ No calendar embed | ❌ Not in JSON | ❌ Missing consent |
...
| Blog with educational content | ⚠️ | 5 posts written; no Elementor blog page template exists |
...
| Sticky navigation header | ❌ | No Elementor header template exists |
| Footer with legal links | ❌ | No footer template exists |
...
| Overture API lead capture | ❌ | Not implemented; requires webhook configuration |
| GDPR consent in all forms | ❌ | Acceptance field missing from all forms |
...
| Build Portfolio page | `XCP_Portfolio_P1_Hero_Filter.json` + `XCP_Portfolio_P2_Gallery_CTA.json` | Critical |
| Test all navigation links | Manual QA | Critical |
| Test contact form email delivery | Submit form, verify email received | Critical |
| Mobile responsiveness check | WordPress Preview + physical device | Critical |

**Deliverable:** All core pages live at xcphotography.co

---

### Phase 2 — Contact Flow & Booking Integration (Week 2)
**Goal:** Full lead capture pipeline from website visitor to booked consultation.

| Task | File / Action | Priority |
|------|--------------|---------|
| Add GDPR consent field to all contact forms | Edit `XCP_ContactForm_Section.json` + `XCP_Contact_P1_Complete.json` | Critical |
| Connect Elementor form to Overture API via webhook | Elementor Pro → Form → Actions → Webhook | Critical |
| Verify Overture receives form submissions | Test with live form submission | Critical |
| Add Overture booking calendar embed to Contact page | Edit `XCP_Contact_P1_Complete.json` or new booking section | High |
| Add booking CTA buttons to Services page | Edit `XCP_Services_P1_Packages.json` | High |
...
### Files Requiring Form Updates Before Import
```
XCP_ContactForm_Section.json    ← Add GDPR field; configure Overture webhook
XCP_Contact_P1_Complete.json    ← Add GDPR field; configure Overture webhook; add booking embed
...
| 7 | Create Privacy Policy page | WordPress → new page | 30 min |
| 8 | Install cookie consent plugin | WordPress plugins | 30 min |

### Must Fix Before Driving Premium Clients to Site

| # | Item | Template/File | Effort |
|---|------|--------------|--------|
| 9 | Connect contact forms to Overture API | Elementor webhook settings | 1 hour |
| 10 | Add real client testimonials (3+ minimum) | Edit testimonial sections | 1 hour |
